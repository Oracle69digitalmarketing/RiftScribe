...
const ShareCard: React.FC<ShareCardProps> = ({ saga, insights, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in" onClick={onClose} >
            <div className="bg-slate-900 border border-amber-400/20 rounded-xl shadow-2xl shadow-black/50 w-full max-w-lg p-6 sm:p-8 m-4 relative" onClick={(e) => e.stopPropagation()} >
                <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-white transition-colors" aria-label="Close" > <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"> <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /> </svg> </button>
                ...
                <div className="mt-6 text-center">
                     <div className="flex flex-wrap justify-center gap-4">
                        <button onClick={handleDownloadImage} className="bg-amber-500 text-slate-900 font-bold py-2 px-6 rounded-md hover:bg-amber-400 transition-colors" > Download Card </button>
                        <button onClick={() => handleShare('twitter')} className="bg-sky-500 text-white font-bold py-2 px-6 rounded-md hover:bg-sky-400 transition-colors" > Share on X </button>
                        <button onClick={() => handleShare('clipboard')} className="bg-slate-600 text-slate-100 font-bold py-2 px-6 rounded-md hover:bg-slate-500 transition-colors w-32" > {copyButtonText} </button>
                    </div>
                </div>
            </div>
        </div>
